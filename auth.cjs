...
async function createAuth(db, { origin = 'http://localhost:9010', secureCookie = false } = {}) {
  ...
  async function newSession(req, res, user, status) {
    ...
    await sessions.insertOne({ userId: user._id, tokenHash: tokenHash(token), createdAt: new Date(), expiresAt: new Date(Date.now() + TTL * 1000) });
  ...
  return async function handleAuth(req, res, pathname) {
    try {
      ...
      if (pathname === '/api/auth/week') {
        ...
        if (req.headers.origin !== origin) throw new HttpError(403, 'Origine refusée.');
        const user = await sessionUser(req);
      ...
      if (req.method !== 'POST') throw new HttpError(405, 'Méthode non autorisée.');
      if (req.headers.origin !== origin) throw new HttpError(403, 'Origine refusée. Ouvre le site à son adresse configurée.');
      if (pathname === '/api/auth/logout') { const token = tokenFrom(req); if (token) await sessions.deleteOne({ tokenHash: tokenHash(token) }); return json(res, 200, { user: null }, { 'Set-Cookie': cookie('', 0) }); }
